...
async function install() {
  ...
  const workspaceRoot = await ask(rl, '  VS Code workspace root (folder you open in VS Code)',
    existingConfig.workspace_root || os.homedir());
...
function sendTest(config) {
  return new Promise((resolve) => {
    ...
    req.write('Installation successful — notifications are working!');
    req.end();
  });
}
